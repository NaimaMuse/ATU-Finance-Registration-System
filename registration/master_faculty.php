...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_faculty'])) {
    ...
    if (empty($faculty_name) || empty($price) || !is_numeric($price) || $price <= 0) {
        ...
        $sql = 'INSERT INTO tblmasterfaculty (faculty_name, price) VALUES (:faculty_name, :price)';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if (empty($faculty_name) || empty($price) || !is_numeric($price) || $price <= 0) {
        ...
        $sql = 'UPDATE tblmasterfaculty SET faculty_name = :faculty_name, price = :price WHERE id = :id';
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':faculty_name', $faculty_name);
        $stmt->bindParam(':price', $price);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    $sql = 'DELETE FROM tblmasterfaculty WHERE id = :id';
    $stmt = $conn->prepare($sql);
...
$sql = 'SELECT * FROM tblmasterfaculty';
$stmt = $conn->query($sql);
...
<body>
    <div class="container mt-5">
        <h2 class="text-center">Master Faculties</h2>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="post" action="" style="display:inline;">
                                    ...
                                    <a href="update_mas_faculty.php?id=<?php echo $faculty['id']; ?>" 
